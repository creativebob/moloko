...
class CreateAttachmentsCategoriesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('attachments_categories', function (Blueprint $table) {
            ...
            $table->foreign('parent_id')->references('id')->on('attachments_categories');

            $table->bigInteger('category_id')->unsigned()->nullable()->comment('Id категории, пишется каждому вложенному пункту');
            $table->foreign('category_id')->references('id')->on('attachments_categories');
    ...
    {
        Schema::dropIfExists('attachments_categories');
    }
}
