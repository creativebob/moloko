...
class CreateServicesFlowsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('services_flows', function (Blueprint $table) {
            $table->bigIncrements('id');

            $table->bigInteger('filial_id')->unsigned()->nullable()->comment('Id отдела');
//            $table->foreign('filial_id')->references('id')->on('departments');

            $table->bigInteger('process_id')->nullable()->unsigned()->comment('Id услуги');
...
//            $table->foreign('initiator_id')->references('id')->on('estimates_services_items');
...
//            $table->foreign('author_id')->references('id')->on('users');
    ...
    {
        Schema::dropIfExists('services_flows');
    }
}
