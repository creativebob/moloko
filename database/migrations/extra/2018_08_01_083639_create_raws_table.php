...
class CreateRawsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('raws', function (Blueprint $table) {
            ...
            $table->integer('raws_product_id')->nullable()->unsigned()->comment('ID сырья');
            ...
            $table->integer('cost')->nullable()->comment('Фиксированная себестоимость сырья (руками)');
            $table->integer('cost_mode')->nullable()->unsigned()->comment('Режим определения себестоимости');

            $table->integer('price')->nullable()->comment('Фиксированная цена сырья (руками)');
