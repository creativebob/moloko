<template>
    <tr
        :class="[{'cmv-archive' : isArchive}, {'article-draft' : isDraft}]"
    >
        <td>{{ index + 1 }}</td>
        <td>{{ item.entity.name }}</td>
        <td>
            {{ item.cmv.article.name }}
            <span v-if="isArchive"> (Архивный)</span>
            <span v-else-if="isDraft"> (Черновик)</span>
        </td>
