<template>
    <div
        class="reveal"
        :id="'modal-estimates_services_item-' + item.id"
        data-reveal
            ...
            <div class="small-12 cell">
                ...
                <div class="grid-x grid-margin-x">
                    ...
                    <div class="cell small-12">
                        <label>Количество
                            <span
                                v-if="isRegistered || item.service.serial === 1"
...
<script>
    export default {
        ...
        data() {
            return {
                isManual: this.$store.getters.SERVICE_ITEM(this.id).is_manual == 1,
                discountPercent: this.$store.getters.SERVICE_ITEM(this.id).discount_percent,
                discountCurrency: this.$store.getters.SERVICE_ITEM(this.id).discount_currency / parseFloat(this.$store.getters.SERVICE_ITEM(this.id).count),
                curCount: parseFloat(this.$store.getters.SERVICE_ITEM(this.id).count),
            }
        },
        mounted() {
            Foundation.reInit($('#modal-estimates_services_item-' + this.id));

            // this.isManual = this.$store.getters.SERVICE_ITEM(this.id).is_manual == 1;
            // this.discountPercent = this.$store.getters.SERVICE_ITEM(this.id).discount_percent;
            // this.discountCurrency = this.$store.getters.SERVICE_ITEM(this.id).discount_currency;
            // this.curCount = this.$store.getters.SERVICE_ITEM(this.id).count;
        },
        computed: {
            item() {
            ...
            count: {
                get() {
                    return parseFloat(this.$store.getters.SERVICE_ITEM(this.id).count);
                },
                set (value) {
                    this.curCount = value;
                }
        ...
        methods: {
            ...
            update() {
                ...
                $('#modal-estimates_services_item-' + this.id).foundation('close');
            },
            reset() {
