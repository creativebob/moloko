                ...
                <table class="search-result-list">
                    <tr v-for="(item, index) in results">
                        <td class="search-result-name">
                            <a
                                :href="'/admin/catalogs_services/' + item.catalogs_service_id + '/prices_services/' + item.id + '/edit'"
                            ><span>{{ item.service.process.name }}</span></a><br>
<!--                            <span class="text-small">{{ item.service.process.manufacturer.company.name }}</span>-->
                        </td>
                        <td  class="search-result-summa">
                            <span>{{ item.total }} руб.</span>
                        </td>
                        <td class="search-result-info">
                            <span v-if="item.service.process.draft" class="draft">Черновик</span>
                        </td>
        ...
        methods: {
            check () {
                ...
                if (this.text.length >= 1) {
                    axios
                        .get('/admin/catalogs_services/' + this.catalogId + '/prices_services/search/' + this.text)
                        .then(response => {
