...
{{-- Таблица --}}
@include('includes.title-content', ['page_info' => $page_info, 'class' => App\ArticlesGroup::class, 'type' => 'table'])
@endsection
            ...
            <tbody data-tbodyId="1" class="tbody-width">
                ...
                <tr class="item @if($articles_group->moderation == 1)no-moderation @endif" id="articles_groups-{{ $articles_group->id }}" data-name="{{ $articles_group->name }}">
                    ...
                    <td class="td-name">
                        ...
                        {{-- %5B%5D --}}
                        ({{ link_to_route('goods.index', $articles_group->articles_count, $parameters = ['articles_group_id' => $articles_group->id], $attributes = ['class' => 'filter_link light-text', 'title' => 'Перейти на список артикулов']) }}) {{ ($articles_group->set_status == true) ? '(Набор)' : '' }}
