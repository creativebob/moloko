            ...
            <div class="top-bar head-content">
                <div class="top-bar-left">
                    ...
                    @can('create', App\Photo::class)
                        <a href="/admin/albums/{{ $album->id }}/photos/create" class="icon-add sprite"></a>
                    @endcan
                </div>
                ...
                @if(!empty($album))
                    ...
                    @if(!empty($album->photos))
                        @foreach($album->photos as $photo)
                            ...
                            <tr class="item @if($photo->moderation == 1)no-moderation @endif"
                                id="photos-{{ $photo->id }}" data-name="{{ $photo->name }}">
                                <td class="td-drop">
                                    <div class="sprite icon-drop"></div>
                                </td>
                                <td class="td-checkbox checkbox">
                                    <input type="checkbox" class="table-check" name="photo_id"
                                           id="check-{{ $photo->id }}"
                                ...
                                <td class="td-photo">
                                    <a href="{{ route('photos.edit', [$album->id, $photo->id]) }}">
                                        <img
                                            src="/storage/{{ $photo->company->id }}/media/albums/{{ $album->id }}/img/small/{{ $photo->name }}"
                                            alt="Фотография альбома">
                                    </a>
                                </td>
                                <td class="td-photo-link">
                                    <ul>
                                        <li>Small
                                            - {{ url('/storage/'.$photo->company_id.'/media/albums/'.$album->id.'/img/small/'.$photo->name) }}</li>
                                        <li>Medium
                                            - {{ url('/storage/'.$photo->company_id.'/media/albums/'.$album->id.'/img/medium/'.$photo->name) }}</li>
                                        <li>Large
                                            - {{ url('/storage/'.$photo->company_id.'/media/albums/'.$album->id.'/img/large/'.$photo->name) }}</li>
                                        <li>Original
                                            - {{ url('/storage/'.$photo->company_id.'/media/albums/'.$album->id.'/img/original/'.$photo->name) }}</li>
                                    </ul>
                                </td>
