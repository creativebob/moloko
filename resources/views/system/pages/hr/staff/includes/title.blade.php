        ...
        <div class="top-bar head-content">
            <div class="top-bar-left">
                ...
                @can('create', App\Staffer::class)
                    <a href="/admin/{{ $pageInfo->alias}}/create" class="icon-add sprite" data-tooltip
                       class="top" tabindex="2" title="Добавить позицию"></a>
        ...
        @if (isset($filter))
            <div class="grid-x">
                ...
                <div class="black-button-group small-12 cell">
                    @isset($archivesCount)
                        @if($archivesCount > 0)
                            <a class="button tiny hollow right dismissed" href="{{ route("staff.archives") }}">Архив: {{ $archivesCount }}</a>
                        @endif
                    @else
                        <a class="button tiny hollow right dismissed" href="{{ route("staff.index") }}">Обычные</a>
                    @endisset
                </div>
            </div>
