...
{{--                            <th>Причина увольнения</th>--}}
                            ...
                            <tr id="">
                                ...
                                <td class="actions">
                                    @if($employee_item->dismissal_date == null)
                                        <a class="button alert tiny" id="employee-dismiss"
                                           data-id="{{ $employee_item->id }}">Уволить</a>
                                    @else
            ...
            @if(($list_user_employees->where('dismissal_date', null)->sum('staffer.rate') < 1) && $vacancies->isNotEmpty())
                @if($employee->exists)
                    <div class="small-12 cell">
                        <a class="button green-button" id="employee-employment"
                           data-user-id="{{ $employee->user->id }}">Трудоустроить</a>
                    </div>
                @else
                    <div class="small-12 cell">
                        <div class="grid-x grid-padding-x inputs">
                            <div class="small-12 medium-8 cell">
                                <label>Вакансия:
                                    {!! Form::select('staffer_id', $vacancies->pluck('position.name', 'id')) !!}
