<script type="application/javascript">
    ...
    $(function() {
        ...
        $(document).on('click', '[data-open="modal-create"]', function() {
            ...
            $.get('/admin/catalogs_services/' + catalog_id + '/catalogs_services_items/create', {
        ...
        $(document).on('click', '.sprite-edit', function() {
            ...
            $.get('/admin/catalogs_services/' + catalog_id + '/catalogs_services_items/' + id + '/edit', function(html) {
                $('#modal').html(html);
        ...
        $(document).on('click', '.submit-edit', function(event) {
            ...
            if (window.submitAjax(form.attr('id'))) {
                ...
                // Ajax запрос
                $.ajax({
                    url: '/admin/catalogs_services/' + catalog_id + '/catalogs_services_items/' + form.find('#item-id').val(),
                    type: "PATCH",
                    data: form.serialize(),
        ...
        $(document).on('click', '.delete-button-ajax', function(event) {
            ...
            // Ajax
            $.ajax({
                url: '/admin/catalogs_services/' + catalog_id + '/catalogs_services_items/' + id,
                type: "DELETE",
                success: function (html) {
                    // alert(html);
