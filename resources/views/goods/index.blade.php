                ...
                <tr class="item @if($cur_goods->moderation == 1)no-moderation @endif @if($cur_goods->article->draft) draft @endif" id="goods-{{ $cur_goods->id }}" data-name="{{ $cur_goods->article->name }}">
                    ...
                    <td>
                        <a href="/admin/goods/{{ $cur_goods->id }}/edit">
                            <img src="{{ getPhotoPathPlugEntity($cur_goods, 'small') }}" alt="{{ isset($cur_goods->article->photo_id) ? $cur_goods->article->name : 'Нет фото' }}">
                        </a>
                    </td>
                    <td class="td-name">
...
@include('tmc.create.scripts', ['entity' => 'goods', 'category_entity' => 'goods_categories'])
