<tr class="item" id="compositions-{{ $composition->id }}" data-name="{{ $composition->name }}">
	...
	<td>
		<div class="wrap-input-table">
			...
			$value = null;
			@endphp
			@if(isset($raws_compositions_values[$composition->id]->pivot->value))
			@php
			$value = $raws_compositions_values[$composition->id]->pivot->value;
			@endphp
			@endif
			...
			{{ Form::text('compositions['.$composition->id.']', $value, ['class'=>'digit-field name-field compact w12 padding-to-placeholder', 'id'=>'1', 'maxlength'=>'7', 'autocomplete'=>'off', 'pattern'=>'[0-9\W\s]{0,10}', 'placeholder'=>'0']) }} 
